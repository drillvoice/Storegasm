...
| `lib/actions/` | Server actions: the browser's way in. Each one resolves the user from the session and validates its arguments (`lib/validation.ts`) before calling `lib/db`. |
